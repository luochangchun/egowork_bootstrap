...
public class UserGroupController extends SupportController {
	...
	public ModelAndView add() {
		return new ModelAndView(this.viewPath + "add");
	}
	...
	public Object delete(@RequestParam("id") final Integer id) {
		...
		dao.update("delete from " + prefix + "user_group_menu where gid =" + id);
	...
	public ModelAndView list() {
		...
		view.addObject("data", commonDao.queryForList("select * from " + prefix + "user_group"));
		return view;
	}
	...
	public Object granting(@RequestParam(value = "mid", defaultValue = "") final String mid,
						   ...
						   @RequestParam(value = "btn", defaultValue = "") final String btn,
						   @RequestParam(value = "gid") final int gid) {

		// 在更新前，先清空以前权限，再更新最新权限。
		dao.update("delete from " + prefix + "user_group_menu where gid=" + gid);

		String sql = "insert into " + prefix + "user_group_menu(gid,mid) values(?,?)";

		if (!StringUtils.isEmpty(mid)) {
			batchUpdate(sql, gid, mid);
		}

		dao.update("delete from " + prefix + "user_group_object where gid=" + gid);

		sql = "insert into " + prefix + "user_group_object(gid,oid,type) value(?,?,'button')";

		if (!StringUtils.isEmpty(btn)) {
			batchUpdate(sql, gid, btn);
		}

		sql = "insert into " + prefix + "user_group_object(gid,oid,type) value(?,?,'category')";
	...
	public ModelAndView grant(@RequestParam("id") final int groupId) {
		...
		sql.append("select * from ").append(prefix).append("user_menu order by sort");
		view.addObject("menus", commonDao.queryForList(sql.toString()));
		view.addObject("gid", groupId);

		sql.delete(0, sql.length()); //清空sql
		sql.append("select * from ").append(prefix).append("category ");
		view.addObject("category", commonDao.queryForList(sql.toString()));

		sql.delete(0, sql.length());//清空sql
		sql.append("select * from ").append(prefix).append("user_button ");
