...
@Service(Services.HUMANRESOURCES)
public class HumanResourcesService extends BaseService
{
	...
	public Page find(final int currentPageNo, final int pageSize, final Map<String, Object> condition)
	{
		...
		final String sql = "select a.*, concat('/cms?type=',c.model,'&id=',CAST(a.id as char),'&time=',DATE_FORMAT(a.time,'%Y%m%d'))  url, c.name  as cname ,c.model from "
				+ config.getPrefix() + "human_resources as a " + "left join " + config.getPrefix() + "category c on c.id = a.cid "
				+ "where a.status in (" + status + ") and a.cid in (" + cid + ") and a.title like ? ESCAPE '!' order by a.id desc";
	...
	{
		final String sql = " select aser.* from " + config.getPrefix() + "apply_service aser order by aser.time desc ";
