...
public class CategoryDataSourceImpl extends SqlDataSource {
	...
	public void generateSql() throws SystemException {
		...
		queryString.append("select ")
				   .append(SQL.ALIAS_MODEL)
				   .append(".*,C.name cname, concat('/cms?','type=")
				   .append(model)
				   .append("','&id=',CAST(M.id as char),'&time=',DATE_FORMAT(M.time,'%Y%m%d')) 'url'")
				   .append(" from ")
				   .append(prefix)
				   .append("category")
				   .append(SQL.ALIAS_CATEGORY)
				   .append(SQL.RIGHT_JOIN)
				   .append(prefix)
				   .append(model)
				   .append(SQL.ALIAS_MODEL)
				   .append(SQL.ON_MCID_E_CID);
...
//  
		...
		if (where != null && !"".equals(where)) {// 如果条件语句存在
			...
			for (int i = 0; i < ws.length; i++) {
				queryString.append(SQL.ALIAS_CATEGORY + ".").append(ws[i]);
				if (i != (ws.length - 1)) {
