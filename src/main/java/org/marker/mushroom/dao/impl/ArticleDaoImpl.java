...
public class ArticleDaoImpl extends DaoEngine implements IArticleDao {
	...
	public boolean update(final Article a) {
		...
		String sql = "update " + prefix + "article" +
			" set cid=?,title=?,keywords=?,description=?,author=?,content=?,status=?,source=?,icon=?,time=?,reject=? where id=?";

		final int status = jdbcTemplate.update(sql, a.getCid(), a.getTitle(), a.getKeywords(), a.getDescription(),
											   a.getAuthor(), a.getContent(), a.getStatus(), a.getSource(), a.getIcon(),
											   a.getTime(), a.getReject(), a.getId());
