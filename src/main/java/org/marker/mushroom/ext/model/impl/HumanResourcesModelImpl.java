...
public class HumanResourcesModelImpl extends ContentModel
{
	...
	public void fetchContent(final Serializable cid)
	{
		...
		final String sql = "select  M.*,C.name cname, concat('/cms?','type=humanResources','&id=',CAST(M.id as char),'&time=',DATE_FORMAT(M.time,'%Y%m%d')) url from "
	...
	public Page doPage(final WebParam param)
	{
		...
		sql.append(
				"select A.*,C.name as cname,concat('type=humanResources&id=',CAST(A.id as char),'&time=',DATE_FORMAT(A.time,'%Y%m%d')) as url from ")
	...
	public StringBuilder doWebFront(final String tableName, final SqlDataSource sqlDataSource)
	{
		...
		final StringBuilder sql = new StringBuilder(
				"select  M.*,C.name cname, concat('/cms?','type=humanResources','&id=',CAST(M.id as char),'&time=',DATE_FORMAT(M.time,'%Y%m%d')) url from "
