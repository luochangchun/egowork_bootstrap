...
public class ArticleModelImpl extends ContentModel
{
	...
	public void fetchContent(final Serializable cid)
	{
		...
		final String sql = "select  M.*,C.name cname, concat('/cms?','type=article','&id=',CAST(M.id as char),'&time=',DATE_FORMAT(M.time,'%Y%m%d')) url from "
				+ prefix + "category C " + "right join " + prefix + "article M on M.cid = C.id  where  M.id=?";
	...
	public StringBuilder doWebFront(final String tableName, final SqlDataSource sqlDataSource)
	{
		...
		final StringBuilder sql = new StringBuilder(
				"select  M.*,C.name cname, concat('/cms?','type=article','&id=',CAST(M.id as char),'&time=',DATE_FORMAT(M.time,'%Y%m%d')) url from "
						+ prefix + "category C " + "right join " + prefix + "article M on M.cid = C.id");
