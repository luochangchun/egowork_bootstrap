 ...
 */
public class EnterpriseModelImpl extends ContentModel {

	public EnterpriseModelImpl() {
		final Map<String, Object> config = new HashMap<>();
		config.put("icon", "images/demo.jpg");
		config.put("name", "入孵企业模型");
		config.put("author", "30");
		config.put("version", "0.1");
		config.put("type", "enterprise");
		config.put("template", "enterprise.html");
	...
	public void fetchContent(final Serializable id) {
		...
		final String sql = "select M.*," + url("enterprise", "M") + " from " + prefix + "enterprise M where M.id=?";
	...
	public Page doPage(final WebParam param) {
		final String sql = "select M.*," + url("enterprise", "M") + " from "
			+ getPrefix() + "enterprise M where 1=1 " + param.extendSql + param.orderSql;
