 ...
 * @version 1.0
 */
public class InvestorModelImpl extends ContentModel
{
	...
	public InvestorModelImpl()
	{
		...
		config.put("name", "投资商模型");
		config.put("author", "dd");
		config.put("version", "0.1");
		config.put("type", "investor");
		config.put("template", "investor.html");
	...
	public void fetchContent(final Serializable cid)
	{
		...
		final String sql = "select  M.*,C.name cname, concat('/cms?','type=investor','&id=',CAST(M.id as char),'&time=',DATE_FORMAT(M.time,'%Y%m%d')) url from "
				+ prefix + "category C " + "right join " + prefix + "investor M on M.cid = C.id  where  M.id=?";
		final Object investor = commonDao.queryForMap(sql, cid);

		commonDao.update("update " + prefix + "investor set views = views+1 where id=?", cid);// 更新浏览量
	...
	public Page doPage(final WebParam param)
	{
		...
		sql.append(
				"select A.*,C.name as cname,concat('type=investor&id=',CAST(A.id as char),'&time=',DATE_FORMAT(A.time,'%Y%m%d')) as url from ")
				.append(prefix).append("investor").append(SQL.QUERY_FOR_ALIAS).append(" join ").append(prefix).append("category")
	...
	public StringBuilder doWebFront(final String tableName, final SqlDataSource sqlDataSource)
	{
		...
		final StringBuilder sql = new StringBuilder(
				"select  M.*,C.name cname, concat('/cms?','type=investor','&id=',CAST(M.id as char),'&time=',DATE_FORMAT(M.time,'%Y%m%d')) url from "
						+ prefix + "category C " + "right join " + prefix + "investor M on M.cid = C.id");
