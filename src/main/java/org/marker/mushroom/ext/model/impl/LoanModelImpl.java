...
public class LoanModelImpl extends ContentModel {
	...
	public void fetchContent(final Serializable cid) {
		...
		final String sql =
			"select  M.*,C.name cname, concat('/cms?','type=loan','&id=',CAST(M.id as char),'&time=',DATE_FORMAT(M.time,"
				+ "'%Y%m%d')) url from "
	...
	public Page doPage(final WebParam param) {
		...
		sql.append(
			"select A.*,C.name as cname,concat('type=loan&id=',CAST(A.id as char),'&time=',DATE_FORMAT(A.time,'%Y%m%d')) as url "
				+ "from ")
