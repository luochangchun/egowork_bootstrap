...
public class MessageContext {
	...
	private Map<String, Properties> data = new HashMap<>();

	/** 系统配置对象 */
	private SystemConfig sysconfig = SystemConfig.getInstance();

	/** 语言列表 */
	private List<Locale> list = new ArrayList<>(2);
	...
	public void reday() throws IOException, ClassNotFoundException, URISyntaxException {
		// 基础路径
		String basePath =
			WebRealPathHolder.REAL_PATH + File.separator + "data" + File.separator + "international" + File.separator;
		...
		// 2. 加载国际化properties文件
		for (Locale l : this.list) {
			String lang = getLang(l);
