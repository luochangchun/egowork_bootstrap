...
//上传图片模态框控制
function frameDialogShow(scale){
	if(scale != undefined)
	{
		options.aspectRatio = scale;
	}
	else
	{
...
function imgChange(){
	...
	if(_val){
		$(".input_for_img .img-box").html("<img src='/" + _val + "' style='display:block;width:100%;height:auto;'>");
	}else{
		$(".input_for_img .img-box").html("<img src='images/timg.png' style='display:block;width:100%;height:auto;'>");
	}
}
//上传图片
function upload(obj)
            ...
            success: function(data) {
                ...
                if(dataJson.status){
                    ...
                    if('101' == dataJson.code){// 表示访问正常页面，但会话失效
                        zoom.showMessageDialog(dataJson.message + "<br/>","消息提示",1500,function(){
                            window.location.href = "login.do";// 回调中刷新界面
                        });
                    }else{
                        zoom.showMessageDialog(dataJson.message + "<br/>","消息提示",1500);
                    }
                }
            },
            error: function(){
...
function uniqueUrl(u_url){
	if(u_url != null && u_url.indexOf("?")==-1){u_url+="?r=";}else{u_url+="&r=";}
	u_url += Math.random();
...
zoom.closeTextareaDialog = function(){
    ...
    if(showTextareaDialog != null) window.document.body.removeChild(showTextareaDialog);
};
zoom.showTextareaDialog = function(content,title,fun){
    ...
    var htmlText = "<div id=\"showTextareaDialog\" class=\"dialog\">"+
        ...
        "	<textarea id=\"showTextareaDialogInput\" style='width: 100%;height: 100px;overflow: auto;resize:none' oninput='textNumCal(this)' data-target='#num' data-max='255'>"+content+"</textarea> <span id='num'></span>"+
        " </div>"+
        "<div class=\"dialogButton\">"+
        "<button id=\"dialogButtonConfirm\">确 定</button> &nbsp; &nbsp; "+
        "<button id=\"dialogButtonCancel\">取 消</button>"+
